...
const Addplayers = ({resetIndex, isLandscape}) => {
        ...
        const [playerNames, setPlayerNames] = useState(() => {
            // Generate automatic player names
        ...
        const [playerNamesAmericano, setPlayerNamesAmericano] = useState(() => {
            // Generate automatic player names
            return Array.from({length: parseInt(tournamentData.players)}, (_, index) => ({
                name: `player${index + 1}`,
                score: 0
    ...
    const [playerTest, setPlayerTest] = useState(() => {
        ...
        useEffect(() => {
            setTournamentData({...tournamentData, playerNames,playerNamesAmericano, playerTest});
        }, [playerNames]);
        ...
        const handleSmash = () => {
                ...
                if (tournamentData.type === "Americano") {
                    setPlayerNamesAmericano(playerNames); // Saves a list clean playerNames-list to be used for americano
                    setPlayerTest(playerNames);
                }
